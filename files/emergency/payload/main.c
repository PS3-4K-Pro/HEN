	...
	*/
	...
	CellFsStat stat;
		...
		if((cellFsStat("/dev_flash/vsh/resource/explore/xmb/pro.xml",&stat)==0))
		{
			...
			{
				// Changes the Plugins folder icon and disables it.
				if(cellFsStat("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/patches/boot_plugins",&stat)==0)
				{
					map_path("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/icons/plugin_folder.png","/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/icons/plugin_warning.png",FLAG_MAX_PRIORITY|FLAG_PROTECT);
					map_path("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/xmls/plugins_manager/plugins_manager_xai_HFW_items.xml","/dev_flash/vsh/resource/explore/xmb/pro.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
				
				// Changes the Online game server warning to a custom.
				if(cellFsStat("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/xmls/online_server/sub_items/warning.xml",&stat)==0)
				{
					map_path("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/xmls/online_server/sub_items/warning.xml","/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/xmls/online_server/sub_items/warning_remap.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}

				// Enables Flash Games (Hide when external data is enabled)
				if((cellFsStat("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/online/flash_games/flash_games.xml",&stat)==0))
				{
					map_path("/dev_hdd0/hen/flash_games.xml","/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/online/flash_games/flash_games.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
				
				// Enables Unofficial Stores (Hide when external data is enabled)
				if((cellFsStat("/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/online/unofficial_stores/unofficial_stores.xml",&stat)==0))
				{
					map_path("/dev_hdd0/hen/unofficial_stores.xml","/dev_hdd0/game/PS34KPROX/USRDIR/toolbox/online/unofficial_stores/unofficial_stores.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
				
				// Switches the HEN enabler to custom What's New.
				if((cellFsStat("/dev_flash/hen/xml/hen_enabled.xml",&stat)==0))
				{
					map_path("/dev_hdd0/game/PS34KPROX/HEN/hen_enable.xml","/dev_flash/hen/xml/hen_enabled.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
			}
			else if((cellFsStat("/dev_flash/vsh/etc/lite.txt",&stat)==0))
			{
				...
				{
					map_path("/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/icons/plugin_folder.png","/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/icons/plugin_warning.png",FLAG_MAX_PRIORITY|FLAG_PROTECT);
					map_path("/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/xmls/plugins_manager/plugins_manager_xai_HFW_items.xml","/dev_flash/vsh/resource/explore/xmb/pro.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
				
				// Changes the Online game server warning to a custom.
				if(cellFsStat("/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/xmls/online_server/sub_items/warning.xml",&stat)==0)
				{
					map_path("/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/xmls/online_server/sub_items/warning.xml","/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/xmls/online_server/sub_items/warning_remap.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}

				// Enables Flash Games (Hide when external data is enabled)
				if((cellFsStat("/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/online/flash_games/flash_games.xml",&stat)==0))
				{
					map_path("/dev_hdd0/hen/flash_games.xml","/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/online/flash_games/flash_games.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
				
				// Enables Unofficial Stores (Hide when external data is enabled)
				if((cellFsStat("/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/online/unofficial_stores/unofficial_stores.xml",&stat)==0))
				{
					map_path("/dev_hdd0/hen/unofficial_stores.xml","/dev_hdd0/game/PS34KPROL/USRDIR/toolbox/online/unofficial_stores/unofficial_stores.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
				
				// Switches the HEN enabler to custom What's New.
				if((cellFsStat("/dev_flash/hen/xml/hen_enabled.xml",&stat)==0))
				{
					map_path("/dev_hdd0/game/PS34KPROL/HEN/hen_enable.xml","/dev_flash/hen/xml/hen_enabled.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
				}
			}
			...
			// Enables custom What's New, PlayStation® Billboards and DEX Trophy features on CEX. (Extra: Install PKG on non QA Flagged systems, Disables Hidden Trophies on XMB™, Multi-language for hard codded debug items, Anti-brick patches and Support for case insensitivity in File Explorer for previewing files)
			...
			// Disables Hidden Trophies In-Game
			if((cellFsStat("/dev_flash/vsh/resource/AAA/explore_category_psn.sprx",&stat)==0))
			{
				map_path("/dev_flash/vsh/module/explore_category_psn.sprx","/dev_flash/vsh/resource/AAA/explore_category_psn.sprx",FLAG_MAX_PRIORITY|FLAG_PROTECT);
			}
			...
			// Enables PlayStation®Home folder.
			if((cellFsStat("/dev_flash/vsh/resource/explore/xmb/category_playstation_home.xml",&stat)==0))
			{
				map_path("/dev_hdd0/hen/category_playstation_home.xml","/dev_flash/vsh/resource/explore/xmb/category_playstation_home.xml",FLAG_MAX_PRIORITY|FLAG_PROTECT);
			}
			...
			// DLNA Server on Network Category.
			if((cellFsStat("/dev_flash/vsh/resource/AAA/sysconf_plugin.sprx",&stat)==0))
			{
				map_path("/dev_flash/vsh/module/sysconf_plugin.sprx","/dev_flash/vsh/resource/AAA/sysconf_plugin.sprx",FLAG_MAX_PRIORITY|FLAG_PROTECT);
			}

			// Removes header text on URL download screen.
			if((cellFsStat("/dev_flash/vsh/resource/AAA/download_plugin.sprx",&stat)==0))
			{
	...
	{
		#define PLUGIN_1	"/dev_hdd0/plugins/webftp_server"
		#define PLUGIN_2	"/dev_hdd0/plugins/EBOOTLoader"
		#define PLUGIN_3	"/dev_hdd0/plugins/PS3Lock/ps3lock_vsh"
		#define PLUGIN_4	"/dev_hdd0/plugins/roulette_vsh_menu"
		#define PLUGIN_5	"/dev_hdd0/plugins/gameboot_cycle"
		#define PLUGIN_6	"/dev_hdd0/plugins/openps3ftp"
		#define PLUGIN_7	"/dev_hdd0/plugins/ps3_menu/ps3_vsh_menu"
		#define PLUGIN_8	"/dev_hdd0/plugins/ps3xpad/xpad_vsh"
		#define PLUGIN_9	"/dev_hdd0/plugins/psnpatch"
		#define PLUGIN_10	"/dev_hdd0/plugins/rainbow_xmb"
		#define PLUGIN_11	"/dev_hdd0/plugins/slaunch"
		#define PLUGIN_12	"/dev_hdd0/plugins/trophy_unlocker"
		#define PLUGIN_13	"/dev_hdd0/plugins/fps_counter"
		
		cellFsRename(PLUGIN_1 ".off", PLUGIN_1 ".sprx");
		cellFsRename(PLUGIN_2 ".sprx", PLUGIN_2 ".off");
		cellFsRename(PLUGIN_3 ".sprx", PLUGIN_3 ".off");
		cellFsRename(PLUGIN_4 ".sprx", PLUGIN_4 ".off");
		cellFsRename(PLUGIN_5 ".sprx", PLUGIN_5 ".off");
		cellFsRename(PLUGIN_6 ".sprx", PLUGIN_6 ".off");
		cellFsRename(PLUGIN_7 ".sprx", PLUGIN_7 ".off");
		cellFsRename(PLUGIN_8 ".sprx", PLUGIN_8 ".off");
		cellFsRename(PLUGIN_9 ".sprx", PLUGIN_9 ".off");
		cellFsRename(PLUGIN_10 ".sprx", PLUGIN_10 ".off");
		cellFsRename(PLUGIN_11 ".sprx", PLUGIN_11 ".off");
		cellFsRename(PLUGIN_12 ".sprx", PLUGIN_12 ".off");
		cellFsRename(PLUGIN_13 ".sprx", PLUGIN_13 ".off");
