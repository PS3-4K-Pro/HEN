...
static char rap_bin_sources[3][40] =
{
	"/dev_usb000/exdata/rap.bin",
	"/dev_usb001/exdata/rap.bin",
	"/dev_hdd0/exdata/rap.bin"
};
...
static int read_rap_bin(const char* content_id, uint8_t *rap_value)
{
	CellFsStat stat;
	...
	for(int i = 0; i < 3; i++)
	{
		sprintf(rap_bin_file, rap_bin_sources[i]);
		if(cellFsStat(rap_bin_file, &stat) == CELL_FS_SUCCEEDED)
			break;
	}

	if(cellFsOpen(rap_bin_file, CELL_FS_O_RDONLY, &fd, 0, NULL, 0) == CELL_FS_SUCCEEDED)
	{
		uint8_t *buffer_content_id = buffer + 0x10;
		while(cellFsRead(fd, buffer, 0x50, &nread) == CELL_FS_SUCCEEDED)
		{
			if((int)!nread)
				break;

			if(memcmp(buffer_content_id, content_id, CONTENTID_SIZE) == SUCCEEDED)
			{
...
void make_rif(const char *path)
{
	...
	if(!strncmp(path, "/dev_hdd0/home/", 15) && !strcmp(path + strlen(path) - 4, ".rif"))
	{
		...
		if(!is_pslauncher)
		{
			...
			{
				found_rap_in_bin = read_rap_bin(content_id, rap); // Check if rap.bin file exists
		...
		int fd;
		if(!found_rap_in_bin || is_pslauncher || cellFsOpen((found_rap_in_bin) ? rap_path : rap_bin_file, CELL_FS_O_RDONLY, &fd, 0666, NULL, 0) == CELL_FS_SUCCEEDED)
		{
			uint64_t nread = 0;
