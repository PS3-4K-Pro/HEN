...
void map_first_slot(char *old, char *newp)
{
	...
	map_table[0].newpath = alloc(MAX_PATH_NEW, 0x27);
	strncpy(map_table[0].newpath, newp, MAX_PATH_NEW-1);
...
static void init_map_entry(uint8_t index)
{
	...
	if (map_table[index].flags & FLAG_COPY)
		dealloc(map_table[index].oldpath, 0x27);
	dealloc(map_table[index].newpath, 0x27);
...
int map_path(char *oldpath, char *newpath, uint32_t flags)
{
	...
	for (i = 1; i < MAX_TABLE_ENTRIES; i++)
	{
		if (map_table[i].oldpath)
		{
			if (strcmp(oldpath, map_table[i].oldpath) == 0)
			{
				...
				{
					strncpy(map_table[i].newpath, newpath, MAX_PATH_NEW-1);
					map_table[i].newpath[MAX_PATH_NEW-1] = 0;
	...
	if (i == MAX_TABLE_ENTRIES)
	{
		...
		map_table[firstfree].newpath = alloc(MAX_PATH_NEW, 0x27);
		strncpy(map_table[firstfree].newpath, newpath, MAX_PATH_NEW-1);
		map_table[firstfree].newpath[MAX_PATH_NEW-1] = 0;
